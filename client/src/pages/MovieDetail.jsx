...
// Change this in Vercel using the VITE_API_URL environment variable.
// Falls back to your current backend so the app keeps working.
const API_URL =
...
function MovieDetail() {
      ...
      try {
        ...
        if (data.imdb_id) {
          const streamsRes = await fetch(
            `${API_URL}/api/streams/${data.imdb_id}`,
            { signal: controller.signal }
          )
  ...
  return (
    <div className="movie-detail-page">
      ...
      {nowPlaying && (
        <div
          className="movie-player-block"
          ref={playerRef}
        >
          <h2 className="movie-player-title">
      ...
      <div className="movie-detail-sources">
        <h2 className="movie-detail-sources-title">
          Sources
        </h2>
        ...
        {sourcesLoading ? (
          ...
          <p>
            No addons installed yet. Add some from
          ...
          sources.map((source) => (
            ...
            >
              <h3 className="source-addon-name">
                {source.addon_name}
              </h3>
              ...
              {source.error ? (
                ...
                <ul className="source-stream-list">
                  {source.streams.map((stream, i) => {
                    ...
                    const playable =
                      isPlayableUrl(stream)

                    return (
                      <li
                        key={i}
                        className="source-stream-item"
                      >
                        {playable ? (
                          <button
                          ...
                          <span className="source-stream-unlinked">
                            {label}
                          </span>
                        )}
                      </li>
